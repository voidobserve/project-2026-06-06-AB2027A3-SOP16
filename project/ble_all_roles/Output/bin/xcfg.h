...
typedef struct __attribute__((packed)) _xcfg_cb_t {
    ...
    u8 can_id_byte_1;                           //CAN_ID 第一个字节
    u8 can_id_byte_2;                           //CAN_ID 第二个字节
    u8 can_cmd_index;                           //CAN控制命令所在位置
    u8 can_cmd_left_front_index;                //左前门打开对应的数据位
    u8 can_cmd_left_rear_index;                 //左后门打开对应的数据位
    u8 can_cmd_right_front_index;               //右前门打开对应的数据位
    u8 can_cmd_right_rear_index;                //右后门打开对应的数据位
} xcfg_cb_t;
