...
static uint8_t  brightness; ///< Strip brightness 0-255 (stored as +1)
    ...
    || WS2812_NEO_TYPE == NEO_RBG \
    || WS2812_NEO_TYPE == NEO_GRB \
    || WS2812_NEO_TYPE == NEO_GBR \
    || WS2812_NEO_TYPE == NEO_BRG \
    || WS2812_NEO_TYPE == NEO_BGR)
static uint8_t  pixels[(WS2812_LED_NUM_MAX + 1) * 3];     ///< Holds LED color values (3 or 4 bytes each)   配置RAM大小
                                                          ///总数+1为了避免“实际灯带长度超过设定长度时, 超出位置会有数据穿过导致亮灯”这个问题, 最后一个固定设置0x000000
#else
static uint8_t  pixels[(WS2812_LED_NUM_MAX + 1) * 4];
#endif
...
void Adafruit_NeoPixel_init(uint16_t pixel_num, neoPixelType type)
{
    ...
    */
    ...
    ///总数+1为了避免“实际灯带长度超过设定长度时, 超出位置会有数据穿过导致亮灯”这个问题, 最后一个固定设置0x000000
    if ((numBytes + ((wOffset == rOffset) ? 3 : 4)) == sizeof(pixels))
        pixels[numBytes] = 0x00; pixels[numBytes + 1] = 0x00; pixels[numBytes + 2] = 0x00;
}
