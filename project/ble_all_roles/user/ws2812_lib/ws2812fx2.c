...
void WS2812FX_init(uint16_t num_leds, uint8_t type)
{
    ...
    for (u8 index = 0; index < _active_segments_len; index++)
        WS2812FX_setSpeed_seg(index, DEFAULT_SPEED);
...
void WS2812FX_show_cover_ptr(void (*customShowFunc)(void))
{
    customShow = customShowFunc;
}

void *WS2812FX_show_get_ptr(void)
{
    return customShow;
}
